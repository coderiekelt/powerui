...
namespace JavaScript.Compiler
{
	...
	internal sealed class NameExpression : Expression, IReferenceExpression
	{
		...
		/// <summary>
		/// True if this name expression is a reference to a type (e.g. for static methods).
		/// </summary>
		public bool TypeReference{
			get{
				PropertyVariable pv=(_Variable as PropertyVariable);
				return pv!=null && pv.TypeReference;
			}
		}
		...
		internal override void ResolveVariables(OptimizationInfo optimizationInfo)
		{
			...
			while (scope != null){
				...
				if (_Variable != null){
					...
					if(scope != this.Scope && scope is DeclarativeScope){
						// It's from some other non-global scope.
						
						// How many scopes are we lifting it through?
						if(chainSize == 1){
							// One level only:
							_Variable = (Scope as DeclarativeScope).DeclareRemote(_Variable);
						}else{
							
							// Lifting through multiple scopes.
							// Get the scope chain between my scope and scope:
							Scope[] scopes=Scope.GetChain(chainSize);
							
							// Going *backwards*:
							for(int i=chainSize-1;i>=0;i--){
								
								// Declare a remote variable:
								_Variable = (scopes[i] as DeclarativeScope).DeclareRemote(_Variable);
		...
		public void GenerateDelete(ILGenerator generator, OptimizationInfo optimizationInfo)
		{
			...
			if (optimizationInfo.StrictMode == true)
				throw new JavaScriptException(optimizationInfo.Engine, "SyntaxError", string.Format("Cannot delete {0} because deleting a variable or argument is not allowed in strict mode", this.Name), 1, optimizationInfo.SourcePath, optimizationInfo.FunctionName);
		...
		/// </summary>
		/// <param name="generator"> The generator to output the CIL to (optional).</param>
		public void SetupThis(ScriptEngine engine,ILGenerator generator,out Type thisType)
		{
			thisType = engine.GlobalScopeType;
			...
			{
				// No with scopes in the scope chain, use global scope as the "this" value.
				return;
			}
			...
			do
			{
				...
				if (dScope!=null)
				{
					...
					{
						// The variable exists but declarative scopes always produce global scope for
						// the "this" value.
						break;
					}
				}
