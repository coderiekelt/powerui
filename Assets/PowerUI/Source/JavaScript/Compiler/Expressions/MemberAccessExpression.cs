	...
	internal sealed class MemberAccessExpression : OperatorExpression, IReferenceExpression
	{
		...
		internal override void ResolveVariables(OptimizationInfo optimizationInfo){
			...
			if (this.OperatorType == OperatorType.MemberAccess)
			{
				...
				if (rhs == null)
					throw new JavaScriptException(optimizationInfo.Engine, "SyntaxError", "Invalid member access", 1, optimizationInfo.SourcePath, optimizationInfo.FunctionName);
				propertyName = rhs.Name;
				
				// Resolve the 1st operand only:
				GetOperand(0).ResolveVariables(optimizationInfo);
				
			}else{
				
				// Resolve all kids (indexers could contain a variable):
				...
				if (rhs != null)
				{
					...
					if(rhsType==typeof(string)){
						...
						if(literalStr!=null){
							...
							if(propertyName != null && JSArray.ParseArrayIndex(propertyName) != uint.MaxValue){
			...
			if (isArrayIndex == true)
			{
				...
				JavaScript.Prototype proto=optimizationInfo.Engine.Prototypes.Get(lhsType);
			...
			if (ResolvedProperty==null && propertyName != null)
			{
				...
				if(ResolvedProperty==null){
					...
					ResolvedProperty=proto.AddProperty(propertyName,null,PropertyAttributes.FullAccess);
		...
		public void GenerateGet(ILGenerator generator, OptimizationInfo optimizationInfo, bool throwIfUnresolvable)
		{
			...
			if(ResolvedProperty==null){
				...
				if(rhs is NameExpression && OperatorType != OperatorType.Index){
					
					// Just emit the text:
					string name=(rhs as NameExpression).Name;
				...
				if(ResolvedProperty!=null){
					...
					if(ResolvedProperty.HasAccessor && ResolvedProperty.ImportedFrom==null){
		...
		public void GenerateDelete(ILGenerator generator, OptimizationInfo optimizationInfo)
		{
			...
			throw new JavaScriptException(optimizationInfo.Engine, "SyntaxError", "Delete is not supported by Nitro", 1, optimizationInfo.SourcePath, optimizationInfo.FunctionName);
