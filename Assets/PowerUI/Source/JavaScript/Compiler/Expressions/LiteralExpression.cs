...
namespace JavaScript.Compiler
{
	...
	internal sealed class LiteralExpression : Expression
	{
		...
		public override Type GetResultType(OptimizationInfo optimizationInfo)
		{
			...
			// RegExp literal.
			if (this.Value is RegularExpressionLiteral)
				return typeof(JavaScript.RegExp);
			
			// Everything else.
			return Value.GetType();
		}
		
		private Prototype CreatedPrototype;
		...
		internal override void ResolveVariables(OptimizationInfo optimizationInfo){
			...
			if(CreatedPrototype!=null){
				// Already made it! 
				// As it's a literal, stopping here is fine.
				// Normally ResolveVariables should recalc all cached values.
				return;
			}
			...
			// We'll generate a prototype and a custom object type for it:
			Prototype proto=CreatedPrototype;
			...
			foreach (var keyValuePair in properties)
			{
				...
				if (propertyValue is Expression)
				{
					...
					PropertyVariable pv=proto.AddProperty(propertyName,PropertyAttributes.FullAccess,valueType);
		...
		public override void GenerateCode(ILGenerator generator, OptimizationInfo optimizationInfo)
		{
			...
			else if (this.Value is List<KeyValuePair<Expression, Expression>>)
			{
				...
				foreach (var keyValuePair in properties)
				{
					...
					// Get the property:
					PropertyVariable pv=proto.GetProperty(propertyName);
		...
		public override string ToString()
		{
			...
			if (this.Value is string)
				return JavaScript.String.Quote((string)this.Value);
