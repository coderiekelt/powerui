...
namespace JavaScript.Compiler
{
	...
	internal class AssignmentExpression : OperatorExpression
	{
		...
		internal override void ResolveVariables(OptimizationInfo optimizationInfo){
			...
			if(this.OperatorType == OperatorType.Assignment){
				...
				if(rhs is FunctionExpression){
					...
					// Can't use the constant value for functions.
					Type targetType = target.GetResultType(optimizationInfo);
					...
					#endif
						// Apply deleg to subscribe to:
						(rhs as FunctionExpression).DelegateToSubscribeTo = targetType;
				...
				target.TryApplyConstant(optimizationInfo,rhs);
		...
		private void GenerateAssignment(ILGenerator generator, OptimizationInfo optimizationInfo, IReferenceExpression target)
		{
			...
			// Note that all of the others affect the source value in some way
			// so this is the only one which can potentially transfer a constant like this.
			object constValue = rhs.Evaluate();
			
			// If it's undefined or a function, set it to null:
			if(constValue==Undefined.Value || (rhs is FunctionExpression)){
				constValue=null;
			}
