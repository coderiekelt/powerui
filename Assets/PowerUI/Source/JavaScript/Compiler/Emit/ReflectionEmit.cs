...
namespace JavaScript.Compiler
{
	...
	public class ReflectionEmitType : ILTypeBuilder
	{
		...
		public override ILGenerator DefineConstructor(MethodAttributes attribs,Type[] parameters){
			...
			// Add the JSFunctionAttribute to it:
			ctrBuilder.SetCustomAttribute(
				new System.Reflection.Emit.CustomAttributeBuilder(
					ReflectionHelpers.JSFunctionAttribute_Constructor,
					new object[] {
						0 // SourceIndex
					}
				)
		...
		public override ILGenerator DefineMethod(
			...
			// Add the JSFunctionAttribute to methodBuilder:
	...
	internal class ReflectionEmitModuleInfo : ILModuleInfo
	{
		...
		/// </summary>
		private FieldInfo GlobalEngine_;
		/// <summary>
		/// When the script engine starts, it generates a static class to hold general info.
		...
		private FieldInfo GlobalPrototypes_;
		...
		/// <summary>The prototypes set as a static instance.</summary>
		public override FieldInfo GlobalPrototypes{
			get{
				return GlobalPrototypes_;
			}
		}
		
		/// <summary>The global engine as a static instance.</summary>
		public override FieldInfo GlobalEngine{
			get{
		...
		public override void CreateMainType(string uniqueID){
			...
			string filepath=ScriptEngine.CachePath;
		...
		public override void SetupGlobalCache(Prototype globals,object imports)
		{
			if(GlobalType_!=null)
			{
				return;
			...
			ScriptEngine jsRuntime = Runtime as ScriptEngine;
			
			if(jsRuntime!=null){
				Define(typeBuilder,"Prototypes",typeof(PrototypeLookup));
				Define(typeBuilder,"Globals",globals.Type);
			}
			...
			// Get the fields:
			GlobalEngine_=Get("Engine");
			GlobalEngine_.SetValue(null,Runtime.Engine);
			
			GlobalRuntime_=Get("Runtime");
			GlobalRuntime_.SetValue(null,Runtime);
			
			if(jsRuntime!=null){
				GlobalPrototypes_=Get("Prototypes");
				
				// Apply the values:
				GlobalPrototypes_.SetValue(null,jsRuntime.Prototypes);
			}
	...
	public class ReflectionEmitILGenerator : ILGenerator
	{
		...
		/// </summary>
		public override void Complete(string name, ArgVariable[] arguments, Type returnType)
		{
			Return();
		}
		...
		{
			this.generator.Emit(OpCodes.Div_Un);
		}
		...
		{
			this.generator.Emit(OpCodes.Conv_R4);
		}
		...
		public override void Unaligned(byte align){
			this.generator.Emit(OpCodes.Unaligned,align);
		}
		...
		public override void AddressInt8(){
			this.generator.Emit(OpCodes.Ldind_I1);
		}
		...
		public override void AddressInt16(){
			this.generator.Emit(OpCodes.Ldind_I2);
		}
		...
		public override void AddressInt32(){
			this.generator.Emit(OpCodes.Ldind_I4);
		}
		...
		public override void AddressInt64(){
			this.generator.Emit(OpCodes.Ldind_I8);
		}
		...
		public override void AddressSingle(){
			this.generator.Emit(OpCodes.Ldind_R4);
		}
		...
		public override void AddressDouble(){
			this.generator.Emit(OpCodes.Ldind_R8);
		}
...
namespace JavaScript.Compiler{
	...
	public partial class UserDefinedFunction{
		...
		private void Close(ScriptEngine engine){
			...
			ILTypeBuilder mainBuilder = engine.ModuleInfo.MainBuilder;
			
			if(mainBuilder!=null && body.DeclaringType==mainBuilder.Type){
				// Close the main type now:
				hostType=engine.ModuleInfo.Close();
			}else{
				// Not the global method.
				#if AbleToCompile
				// Get the builder:
				var builder=(body.DeclaringType as System.Reflection.Emit.TypeBuilder);
				
				// Get the host type:
				hostType=builder.CreateType();
				#else
				// It's always closed.
				return;
				#endif
...
namespace JavaScript{
	...
	public partial class ScriptEngine{
		...
		/// <summary>Sets up the required scope variables in the given assembly.</summary>
		protected override void SetupScopes(Assembly asm){
			
			// Mark as started so it can directly set the globals:
			HasStarted=true;
			
			// Global variables are held in a group of static fields in this type:
			Type globals = asm.GetType("_global");
			
			if(globals!=null){
				
				// Get global prototype:
				GlobalPrototype = Prototypes.Get(globals);
				
				// Setup standard global scope:
				Prototypes.Setup(null,false);
			...
			if(cache!=null){
				...
				// Apply the values:
				cache.GetField("Engine").SetValue(null,this);
				cache.GetField("Runtime").SetValue(null,this);
				cache.GetField("Prototypes").SetValue(null,Prototypes);
				
				var imports=cache.GetField("Imports");
				
				if(imports!=null){
					imports.SetValue(null,ImportedGlobals);
				}
		...
		public CompiledCode Compile(string source,string uniqueID,string location)
		{
			...
			if(firstTime){
				// Create the prototype lookup:
				Prototypes=new PrototypeLookup(this);
			}
			
			// Try loading from the cache:
			CompiledCode existing = Load(uniqueID);
			...
			#if !AbleToCompile
			throw new Exception("Can't compile JavaScript on this platform!");
			#else
			...
			if(firstTime){
				...
				// Load the prototypes:
				Prototypes.Setup(ImportedGlobals,true);
				
			}
			
			var methodGen = new JavaScript.Compiler.GlobalMethodGenerator(
			...
			// Generate code
			methodGen.GenerateCode(null,(Prototype)null);
			
			// Close any open prototypes:
			Prototypes.CompleteAll();
			
			// Bake the types now:
			moduleGenerationInfo.Close();
			
			// Started! (often just does nothing - that's fine)
			Started();
			
			// Get the main method:
			var udf = methodGen.GeneratedMethods[0];
			
			// Close it:
			udf.Done(this);
			
			// Get the main method body and return it as a script:
			return new CompiledCode(udf.body);
	...
	public partial class Runtime{
		...
		/// </summary>
		internal Compiler.ILModuleInfo ModuleInfo{
			get{
				if (moduleGenerationInfo == null)
				{
					#if AbleToCompile
					moduleGenerationInfo = new Compiler.ReflectionEmitModuleInfo(this);
					#else
		...
		/// Gets or sets information needed to generate the module.
		/// </summary>
		protected Compiler.ILModuleInfo moduleGenerationInfo;
	...
	public partial class Module{
		...
		protected override void SetupScopes(Assembly asm){
			...
			if(cache!=null){
				...
				// Apply the values:
				cache.GetField("Engine").SetValue(null,Engine);
				cache.GetField("Runtime").SetValue(null,this);
		...
		public JavaScript.Compiler.ILGenerator DefineMethod(string name, Type returnType, Type[] parameters){
		...
		public void Compile(string uniqueID){
			...
			ModuleInfo.SetupGlobalCache(null,null);
			...
			// Done!
			gen.Complete("cctor",null,typeof(void));
