...
namespace JavaScript.Compiler
{
	...
	public class OptimizationInfo
	{
		...
		/// <summary>
		/// The URI of the source (if it's known).
		/// </summary>
		...
		public ILLocalVariable GetRegExpVariable(ILGenerator generator, RegularExpressionLiteral literal)
		{
			...
			if (this.regularExpressionVariables.TryGetValue(literal, out variable) == false)
			{
				...
				variable = generator.DeclareVariable(typeof(JavaScript.RegExp));
		...
		/// </summary>
		internal DeclaredVariable ReturnVariable;

		/// <summary>If there's only one return statement, the single return type.</summary>
		internal Type SingleReturnType;
		...
		public void SyntaxError(string message){
			throw new JavaScriptException(Engine, "SyntaxError", message, 1, this.SourcePath, this.FunctionName);
		}
		
		/// <summary>Throws a type error at the current function.</summary>
		public void TypeError(string message){
			throw new JavaScriptException(Engine, "TypeError", message, 1, this.SourcePath, this.FunctionName);
		}
				...
				foreach (var labelName in labelNames)
					foreach (var info in this.breakOrContinueStack)
						if (info.LabelNames != null && info.LabelNames.Contains(labelName) == true)
							throw new JavaScriptException(this.Engine, "SyntaxError", string.Format("Label '{0}' has already been declared", labelName), 1, this.SourcePath, this.FunctionName);
			}
		...
		public ILLabel GetBreakTarget(string labelName)
		{
			if (labelName == null)
			{
				...
				throw new JavaScriptException(this.Engine, "SyntaxError", "Illegal break statement", 1, this.SourcePath, this.FunctionName);
			}
			else
			{
				foreach (var info in this.breakOrContinueStack)
				...
				throw new JavaScriptException(this.Engine, "SyntaxError", string.Format("Undefined label '{0}'", labelName), 1, this.SourcePath, this.FunctionName);
			}
		}
		...
		public ILLabel GetContinueTarget(string labelName)
		{
			if (labelName == null)
			{
				...
				throw new JavaScriptException(this.Engine, "SyntaxError", "Illegal continue statement", 1, this.SourcePath, this.FunctionName);
			}
			else
			{
				foreach (var info in this.breakOrContinueStack)
				{
					if (info.LabelNames != null && info.LabelNames.Contains(labelName) == true)
					{
						if (info.ContinueTarget == null)
							throw new JavaScriptException(this.Engine, "SyntaxError", string.Format("The statement with label '{0}' is not a loop", labelName), 1, this.SourcePath, this.FunctionName);
						return info.ContinueTarget;
					}
				}
				throw new JavaScriptException(this.Engine, "SyntaxError", string.Format("Undefined label '{0}'", labelName), 1, this.SourcePath, this.FunctionName);
