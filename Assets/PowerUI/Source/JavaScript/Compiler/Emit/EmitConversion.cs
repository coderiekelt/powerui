...
namespace JavaScript.Compiler
{
	...
	internal static class EmitConversion
	{
		...
		{
			Convert(generator, fromType, toType, optimizationInfo.SourcePath, optimizationInfo.FunctionName);
		}
		...
		public static void Convert(ILGenerator generator, Type fromType, Type toType, string path, string function)
		{
			...
			else if(toType == typeof(JavaScript.Undefined))
			{
				generator.Pop();
			...
			#if NETFX_CORE
			else if(typeof(Delegate).GetTypeInfo().IsAssignableFrom(toType.GetTypeInfo()))
			#else
			else if(typeof(Delegate).IsAssignableFrom(toType))
			#endif
			{
				
				throw new NotImplementedException(string.Format("C2 primitive type: {0}", toType));
			}
			else
			{
		...
		public static void ToBool(ILGenerator generator, Type fromType)
		{
			...
			if(fromType == typeof(JavaScript.Undefined) || fromType == null){
		...
		public static void ToInteger(ILGenerator generator, Type fromType)
		{
			...
			if(fromType == typeof(JavaScript.Undefined) || fromType == null){
				...
				fromType == typeof(object) || fromType == typeof(ObjectInstance)){
		...
		public static void ToNumber(ILGenerator generator, Type fromType)
		{
			...
			if(fromType == typeof(JavaScript.Undefined))
			{
			...
			else if(fromType == null)
			{
				...
				generator.Pop();
				generator.LoadDouble(0.0);
			}else if(fromType == typeof(JavaScript.Date))
			{
		...
		{
			ToPrototype(generator, fromType, optimizationInfo.SourcePath, optimizationInfo.FunctionName);
		}
		...
		/// <param name="generator"> The IL generator. </param>
		/// <param name="fromType"> The type to convert from. </param>
		public static void ToPrimitive(ILGenerator generator, Type fromType)
		{
			// It's already a prim otherwise:
			if(fromType == typeof(object) || fromType == typeof(Extendable)){
