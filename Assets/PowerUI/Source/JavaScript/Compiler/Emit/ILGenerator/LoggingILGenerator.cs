...
namespace JavaScript.Compiler
{
	...
	public class LoggingILGenerator : ILGenerator
	{
		...
		{
			Log("ret");
			this.generator.Complete(name,arguments,returnType);
			...
			if(arguments!=null){
				for(int i=0;i<arguments.Length;i++){
					...
					args+=arguments[i].Type+" "+arguments[i].Name.ToString();
				}
			}
			
			ScriptEngine.Log("-IL "+name+"("+args+") : "+returnType+"-");
			ScriptEngine.Log(il);
		...
		{
			LogInstruction(instruction, JavaScript.String.Quote(stringConstant));
		}
