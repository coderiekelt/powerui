...
namespace JavaScript.Compiler
{
	...
	internal class FunctionToken : KeywordToken
	{
		...
		public override Statement ParseNoNewContext(Parser parser)
		{
            ...
            FunctionExpression expr = parser.ParseFunction(
				FunctionDeclarationType.Declaration,
				parser.initialScope,
				functionName
			);
			
			// Note that the callee has a special case for FunctionExpression.
			// It must "hoist" the declaration to the top of 
			// the scope and emit a generic delegate.
            return parser.Labels(new ExpressionStatement(expr));
		}
