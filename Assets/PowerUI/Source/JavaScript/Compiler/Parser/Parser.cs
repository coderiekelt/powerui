...
namespace JavaScript.Compiler
{
	...
	internal sealed class Parser
	{
		...
		public Parser(ScriptEngine engine, Lexer lexer, Scope initialScope,OptimizationInfo optimizationInfo, CodeContext context)
		{
			if (engine == null)
			...
			this.StrictMode = false;//options.ForceStrictMode;
        ...
        internal List<ArgVariable> ParseFunctionArguments(Token endToken){
			...
			while (true)
			{
				...
				else if (this.nextToken == PunctuatorToken.Colon)
				{
					...
					Prototype proto=this.engine.Prototypes.Get(argumentType);
		...
		internal FunctionExpression ParseFunction(FunctionDeclarationType functionType, Scope parentScope, string functionName)
        {
            ...
            DeclarativeScope scope = DeclarativeScope.CreateFunctionScope(parentScope, arguments);
			...
			FunctionMethodGenerator context = new FunctionMethodGenerator(this.engine, scope, functionName, arguments, body,
				SourcePath);
			context.MethodOptimizationHints = newMethodOptimizationHints;
			expr.SetContext(context);
			
			if(includeNameInScope && functionName!=""){
				
				// Add the non-anony function to the parent scope next.
				// We do it here in the parser to 'hoist' the function. Get the type:
				Type funcType=expr.GetResultType(OptimizationInfo);
				
				// Add now:
				Variable variable=parentScope.AddVariable(functionName, funcType, expr);
				
				// Set it in the expression too:
				expr.ParentVar = variable;
				
				// Try setting a constant value:
				variable.TrySetConstant(context);
