...
namespace JavaScript.Compiler
{
	...
	public class ObjectScope : Scope
	{
		...
		internal override Variable GetVariable(string name){
			...
			// Get the property:
			Variable variable=ScopePrototype.GetProperty(name);
			
			if(variable==null && ParentScope==null){
				
				// This is the global scope and a variable wasn't found.
				// If allowed to, try searching for it:
				return Engine.GlobalNotFound(name);
		...
		internal override Variable AddVariable(string name,Type type, Expression valueAtTopOfScope)
		{
			...
			// Add it now:
			pv=ScopePrototype.AddProperty(name,null,PropertyAttributes.FullAccess);
