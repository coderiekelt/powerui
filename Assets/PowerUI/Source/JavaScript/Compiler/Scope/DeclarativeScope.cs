...
namespace JavaScript.Compiler
{
	...
	public class DeclarativeScope : Scope
	{
		...
		/// <summary>True if we have any 'remote' vars in this scope.
		/// Those are ones imported from a parent scope.</summary>
		internal bool HasRemote;
		...
		internal static DeclarativeScope CreateFunctionScope(Scope parentScope, List<ArgVariable> args)
		{
			if (args == null)
		...
		/// <summary>Gets a list of remote vars. Null if there aren't any.</summary>
		public List<RemoteVariable> GetRemotes(){
			
			if(!HasRemote){
				return null;
			}
			
			List<RemoteVariable> rv=new List<RemoteVariable>();
			
			foreach(KeyValuePair<string,Variable> kvp in variables){
			
				RemoteVariable rem=kvp.Value as RemoteVariable;
		...
		public override void Reset(){
			...
			foreach(KeyValuePair<string,Variable> kvp in variables){
				...
				// Ensure store and type is cleared out:
				dec.Reset();
		...
		internal Variable Declare(Variable variable)
		{
			variables[variable.Name]=variable;
			return variable;
		}
		
		/// <summary>Declare a remote variable. They're vars from some other function scope.</summary>
		internal RemoteVariable DeclareRemote(Variable original)
		{
			
			// If it's an arg variable, we must convert it to a local:
			ArgVariable av=(original as ArgVariable);
			
			if(av!=null){
				original=av.ConvertToLocal();
			}
			
			HasRemote=true;
			
			RemoteVariable h=new RemoteVariable(original.Name,original);
			variables[h.Name]=h;
			return h;
		}
		...
		internal override Variable AddVariable(string name,Type type,Expression valueAtTopOfScope)
		{
			...
			if (variable == null || !(variable is DeclaredVariable))
			{
				...
				dVariable = new DeclaredVariable(name);
				variable = dVariable;
