	...
	public static class TypeComparer
	{
		...
		public new static bool Equals(object x, object y)
		{
			...
			if (y is ConcatenatedString)
				y = y.ToString();
			if ((x == Undefined.Value && y == null) || (x == null && y == Undefined.Value))
				return true;
			if(x==null || y==null){
				// Only true if they're both null:
				return x==y;
			}
			...
			if (TypeUtilities.IsNumeric(x) && y is JavaScript.Extendable)
				return Equals(x, TypeConverter.ToPrimitive(y));
			if (TypeUtilities.IsString(x) && y is JavaScript.Extendable)
				return Equals(x, TypeConverter.ToPrimitive(y));
			if (x is JavaScript.Extendable && TypeUtilities.IsNumeric(y))
				return Equals(TypeConverter.ToPrimitive(x), y);
			if (x is JavaScript.Extendable && TypeUtilities.IsString(y))
				return Equals(TypeConverter.ToPrimitive(x), y);
		...
		public static bool StrictEquals(object x, object y)
		{
			...
			#if NETFX_CORE
			if(outXType.GetTypeInfo().IsValueType){
			#else
		...
		{
			x = TypeConverter.ToPrimitive(x);
			y = TypeConverter.ToPrimitive(y);
