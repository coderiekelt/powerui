	...
	public class MathObject
	{
		...
		public static double Hypot(params double[] numbers)
		{
			...
			if (numbers.Length == 1)
				return numbers[0];
			if (numbers.Length == 2)
				return Hypot2(numbers[0], numbers[1]);

			double result = Hypot2(numbers[0], numbers[1]);
			for (int i = 2; i < numbers.Length; i++)
				result = Hypot2(result, numbers[i]);
			return result;
		}
		...
		private static double Hypot2(double number1, double number2)
		{
