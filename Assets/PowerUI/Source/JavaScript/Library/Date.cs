...
namespace JavaScript
{
	...
	public partial class Date
	{
		...
		public static object ToJSON(ScriptEngine engine, object thisObj, string key)
		{
			var number = TypeConverter.ToNumber(TypeConverter.ToPrimitive(thisObj));
			if (double.IsInfinity((double)number) || double.IsNaN((double)number)){
				return null;
			}
			
			Prototype proto=engine.Prototypes.Get(thisObj);
			
			if(proto==null){
				return null;
			}
			
			return proto.CallMemberFunctionOn(thisObj, "toISOString",false);
		}
		...
		private static string ToTimeZoneString(DateTime dateTime)
		{
			...
			try{
				...
				// Local timezone is broken on some Unity versions.
				// Just use UTC:
				return "GMT+0000 (UTC)";
			}
