...
namespace JavaScript
{
	...
	public partial class DataView
	{
		...
		internal DataView(ScriptEngine engine, ArrayBuffer buffer, int byteOffset, int byteLength)
		{
			Engine = engine;
			this.buffer = buffer;
		...
		/// Throws an error.
		/// </summary>
		public static object OnCall(ScriptEngine engine)
		{
		...
		public static DataView OnConstruct(ScriptEngine engine,ArrayBuffer buffer, int byteOffset, int byteLength)
		{
			if (buffer == null)
			...
			int byteLengthValue = byteLength!=0 ? byteLength : buffer.ByteLength - byteOffset;
			if (byteOffset + byteLengthValue > buffer.ByteLength)
				throw new JavaScriptException(engine, "RangeError", "Invalid data view length.");
			return new DataView(engine, buffer, byteOffset, byteLengthValue);
		}
