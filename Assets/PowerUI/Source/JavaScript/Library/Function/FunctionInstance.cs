...
namespace JavaScript
{
	...
	public abstract partial class FunctionInstance
	{
		...
		internal Compiler.FunctionMethodGenerator Generator;
		...
		public bool HasInstance(ScriptEngine engine,object instance)
		{
			...
			#if NETFX_CORE
			return GetInstanceType(engine).GetTypeInfo().IsAssignableFrom(type.GetTypeInfo());
			#else
		...
		public object Apply(ScriptEngine engine,object thisObj, object arguments)
		{
			...
			if (arguments == null || arguments == Undefined.Value)
				argumentsArray = new object[0];
			else
			{
				JSArray argumentsObject = arguments as JSArray;
				...
				int arrayLength = JSArray.get_Length(engine,argumentsObject);
		...
		{
			throw new Exception("Can't dynamically compile functions on this platform.");
			
			/*
			// Passing no arguments results in an empty function.
			if (argumentsAndBody.Length == 0)
				return Compiler.UserDefinedFunction.Create(engine, "anonymous", null, string.Empty);

			// Split any comma-delimited names.
			List<Compiler.ArgVariable> args = new List<Compiler.ArgVariable>();
			args.Add(new Compiler.ArgVariable("this"));
			...
			for (int i = 0; i < argumentsAndBody.Length - 1; i++)
			{
				...
				for (int j = 0; j < splitNames.Length; j++)
				{
					...
					string argumentName = JavaScript.String.Trim(splitNames[j]);
					...
					args.Add(new Compiler.ArgVariable(argumentName));
			...
			return Compiler.UserDefinedFunction.Create(engine, "anonymous", args, argumentsAndBody[argumentsAndBody.Length - 1]);
			*/
		}
	}
}
